...
async fn move_to_trash(
    ...
    Err("A limpeza assistida ainda não está disponível no protótipo Android. A v0.8 opera somente leitura.".to_string())
}
...
async fn restore_cleanup(
    ...
    Err("O desfazer da limpeza não é usado no protótipo Android porque a v0.8 não remove arquivos.".to_string())
}
